...
const Project = ({ title, image, description, github, demo, tech }: any) => {
  ...
  return (
    <div className="h-full md:h-[450px] lg:h-full w-full flex flex-col sm:flex-row justify-center items-center mt-2 sm:mt-0">
      {/* left side project */}
      <div className="w-full sm:w-1/3 h-64 sm:h-full border-4 border-black dark:border-gray-200 flex flex-col justify-between items-center py-4 2xl:py-10 sm:px-2 lg:px-4 mr-0 sm:mr-2 bg-[#acafb5] dark:bg-neutral-800 mb-6 sm:mb-0">
        ...
        <ul className="m-0 p-0 w-full flex justify-center items-center">
          <li>
            <FaGithub
              className="w-8 h-8 mr-3 cursor-pointer dark:hover:text-black"
              onClick={() => gitHubClickHandler()}
            />
          </li>
          <li>
            <HiExternalLink
              className="w-8 h-8 ml-3 cursor-pointer dark:hover:text-black"
